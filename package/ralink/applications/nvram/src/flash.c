...
int32_t flash_read(char *buf, off_t from, size_t len)
{
    ...
    if (fd < 0)
    {
        DEBUG("Could not open mtd device\n");
        return -1;
    }

    if (ioctl(fd, MEMGETINFO, &info))
    {
        DEBUG("Could not get mtd device info\n");
        close(fd);
        return -1;
    }
    if (len > info.size)
    {
        DEBUG("Too many bytes - %zu > %u bytes\n", len, info.erasesize);
        close(fd);
        return -1;
    }
